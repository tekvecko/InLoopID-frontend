...
# 1. Záloha
with open(file_path + ".bak_parser_ui", 'w') as f:
    f.write(content)

# 2. Výměna funkce parseru za přesnější verzi omezující "vysávání" odpadních sloupců
...
new_parser = """const handleParseText = () => {
      ...
      lines.forEach((line) => {
          ...
          if (foundEmail && !seenEmails.has(foundEmail)) {
              ...
              // Zastavíme vysavač: vezmeme jen první dva sloupce, zbytek (role, extra data) ignorujeme
...
content = re.sub(parser_pattern, new_parser, content, flags=re.DOTALL)
...
print("[+] Úspěšně dokončeno: Parser je nyní přesnější a e-mail je plně editovatelný!")

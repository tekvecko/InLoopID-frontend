...
import { Building, Key, CheckCircle, Copy, ArrowRight, AlertTriangle, CreditCard, Check, Zap, Shield } from 'lucide-react';
...
export const B2BRegister = () => {
    ...
    return (
      <div className="min-h-screen p-6 bg-slate-950 flex flex-col justify-center items-center text-white">
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-6xl w-full">
           ...
           {/* Free Tarif */}
           <div onClick={() => setSelectedPlan('free')} className={`cursor-pointer rounded-3xl p-8 border-2 transition-all duration-300 ${selectedPlan === 'free' ? 'bg-blue-950 border-blue-500 shadow-xl shadow-blue-900/20 transform -translate-y-2' : 'bg-slate-900 border-slate-800 hover:border-slate-700'}`}>
             <h3 className="text-2xl font-bold text-white mb-2">Free</h3>
             <div className="flex items-baseline gap-2 mb-6"><span className="text-4xl font-extrabold">0 Kč</span><span className="text-slate-400">/ měsíc</span></div>
             <ul className="space-y-4 mb-8">
               <li className="flex items-center gap-3"><Check size={20} className="text-slate-400"/> Základní Zero-Knowledge Vault</li>
               <li className="flex items-center gap-3"><Check size={20} className="text-slate-400"/> Až 5 zaměstnanců</li>
               <li className="flex items-center gap-3"><Check size={20} className="text-slate-400"/> Komunitní podpora</li>
             </ul>
             <button onClick={() => setStep(2)} className={`w-full py-4 rounded-xl font-bold transition-colors ${selectedPlan === 'free' ? 'bg-blue-600 text-white hover:bg-blue-500' : 'bg-slate-800 text-slate-300'}`}>Zvolit Free</button>
           </div>
           ...
           <div onClick={() => setSelectedPlan('enterprise')} className={`cursor-pointer relative rounded-3xl p-8 border-2 transition-all duration-300 ${selectedPlan === 'enterprise' ? 'bg-blue-950 border-blue-500 shadow-xl shadow-blue-900/20 transform -translate-y-2' : 'bg-slate-900 border-slate-800 hover:border-slate-700'}`}>
             ...
             <ul className="space-y-4 mb-8">
               ...
               <li className="flex items-center gap-3"><Check size={20} className="text-emerald-400"/> Plně automatizovaný nástup</li>
             </ul>
          ...
          <div className="max-w-xl w-full bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl">
            ...
            <form onSubmit={handleRegister} className="space-y-6">
              ...
              <div className="pt-4 border-t border-slate-800">
                 <button type="submit" className="w-full py-4 bg-blue-600 hover:bg-blue-500 rounded-xl font-bold flex justify-center items-center gap-3"><CreditCard size={20} /> Dokončit registraci a vytvořit Trezor</button>
              </div>
            </form>
          </div>
        </div>
    ...
    return (
      <div className="min-h-screen p-6 bg-slate-950 flex justify-center items-center text-white">
        <div className="max-w-2xl w-full bg-blue-950 border border-blue-900 rounded-3xl p-8 shadow-2xl animate-fade-in text-center">
          ...
          <h2 className="text-3xl font-bold mb-4">Workspace úspěšně aktivován!</h2>
  ...
  return (
      <div className="min-h-screen p-6 bg-slate-950 flex flex-col justify-center items-center text-white">
          ...
          <h2 className="text-2xl font-bold mb-2">Inicializuji workspace...</h2>

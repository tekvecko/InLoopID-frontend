...
# Záchranná záloha pro jistotu
if os.path.exists(file_path):
    os.rename(file_path, file_path + ".bak_expirations")

new_code = """import React, { useState, useEffect } from 'react';
import { ShieldCheck, FileText, UploadCloud, Lock, AlertTriangle, Key, Upload, CheckSquare, Square, Users, FilePlus, Paperclip, X, Link as LinkIcon, Calendar, User, Briefcase, Filter, CheckCircle, Clock, AlertOctagon } from 'lucide-react';
...
export const HRDashboard = () => {
  ...
  // Zvýšená penalizace: Pokud je doba určitá, musí mít datum platnosti!
      ...
      try {
          for (const cand of finalCandidates) {
              if (cand.contractUrl || cand.contractFileBase64) {
                  ...
                  // Přibalení valid_until obálkou pro backend
                  await safeFetch(`${BACKEND_URL}/hr/contracts`, { 
                      method: 'POST', headers: {'Content-Type': 'application/json'}, 
                      body: JSON.stringify({ 
                          tenant_id: tenantId.trim(), email: cand.email, content_hash: mockHash, encrypted_payload: mockEncryptedPayload,
                          valid_until: cand.duration === 'urcita' ? parseDateToISO(cand.validUntil) : null
                      }) 
                  });
              }
          }
  ...
  const handleIssueContract = async (e) => {
      ...
      try {
          ...
          await safeFetch(`${BACKEND_URL}/hr/contracts`, { 
              method: 'POST', headers: {'Content-Type': 'application/json'}, 
              body: JSON.stringify({ 
                  tenant_id: tenantId.trim(), email: contractForm.email, content_hash: mockHash, encrypted_payload: mockEncryptedPayload,
  ...
  return (
    <div className="min-h-screen p-6 bg-slate-950 text-white">
      <div className="max-w-6xl mx-auto space-y-6">
                ...
                <div className="max-h-[550px] overflow-y-auto space-y-4 mb-6 pr-2">
                    {getSortedCandidates().map((cand) => {
                        ...
                        <div key={cand.id} className={`p-5 rounded-2xl border transition-colors ${!cand.selected ? 'bg-slate-950/50 border-slate-800 opacity-50' : hasProblem ? 'bg-rose-950/20 border-rose-900/50' : 'bg-slate-950 border-slate-700'}`}>
                            ...
                            {/* PŘEPÍNAČ: DOBA URČITÁ / NEURČITÁ */}
                            {cand.selected && (
                                <div className="pl-10 mb-4 flex gap-4">
                                    <div className="flex bg-slate-900 border border-slate-800 rounded-lg overflow-hidden focus-within:border-blue-500 w-full md:w-1/3">
                                        <select value={cand.duration} onChange={(e) => {
                                            const val = e.target.value;
                                            let vu = cand.validUntil;
                                            if (val === 'urcita') vu = addOneYear(cand.startDate);
                                            else vu = '';
                                            setParsedCandidates(prev => prev.map(c => c.id === cand.id ? { ...c, duration: val, validUntil: vu } : c));
                                        }} className="w-full bg-transparent px-3 py-2 text-sm outline-none font-bold text-slate-300 appearance-none cursor-pointer">
                                            <option value="neurcita">Doba neurčitá</option>
                                            <option value="urcita">Doba určitá (+ Rok)</option>
                                        </select>
                                    </div>
                                    {cand.duration === 'urcita' && (
                                        <div className={`flex bg-slate-900 border rounded-lg overflow-hidden focus-within:border-rose-500 w-full md:w-1/3 ${!cand.validUntil ? 'border-rose-500/50' : 'border-rose-900/50'}`}>
                                            <div className="bg-rose-900/30 px-3 flex items-center"><Clock size={14} className="text-rose-400"/></div>
                                            <input type="text" value={cand.validUntil} onChange={(e) => updateCand(cand.id, 'validUntil', e.target.value)} placeholder="Platnost do (DD.MM.RRRR)" className="w-full bg-transparent px-3 py-2 text-sm outline-none text-rose-400 font-bold" />
                                        </div>
                                    )}
                                </div>
                            )}
        ...
        {activeTab === 'contracts' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in"><div className="bg-slate-900 p-8 rounded-3xl border border-slate-800"><h2 className="text-xl font-bold mb-6 flex items-center gap-3"><FilePlus className="text-emerald-500"/> Generátor Dokumentů</h2><form onSubmit={handleIssueContract} className="space-y-4"><div><label className="block text-xs font-bold text-slate-500 mb-1 uppercase tracking-wider">Zaměstnanec (E-mail)</label><input type="email" value={contractForm.email} onChange={e => setContractForm({...contractForm, email: e.target.value})} placeholder="Např. jan.novak@firma.cz" required className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white outline-none focus:border-blue-500" /></div>
            
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="block text-xs font-bold text-slate-500 mb-1 uppercase tracking-wider">Nástup</label>
                    <input type="text" value={contractForm.startDate} onChange={e => setContractForm({...contractForm, startDate: e.target.value})} placeholder="DD.MM.RRRR" className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white outline-none focus:border-blue-500" />
                </div>
                <div>
                     <label className="block text-xs font-bold text-slate-500 mb-1 uppercase tracking-wider">Trvání</label>
                     <select value={contractForm.duration} onChange={e => {
                         const val = e.target.value;
                         let vu = contractForm.validUntil;
                         if (val === 'urcita') vu = addOneYear(contractForm.startDate);
                         else vu = '';
                         setContractForm({...contractForm, duration: val, validUntil: vu});
                     }} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white outline-none focus:border-blue-500 cursor-pointer">
                         <option value="neurcita">Doba neurčitá</option>
                         <option value="urcita">Doba určitá (+ Rok)</option>
                     </select>
                </div>
            ...
            {contractForm.duration === 'urcita' && (
                <div>
                    <label className="block text-xs font-bold text-rose-400 mb-1 uppercase tracking-wider">Konec platnosti (Expirace)</label>
                    <input type="text" value={contractForm.validUntil} onChange={e => setContractForm({...contractForm, validUntil: e.target.value})} placeholder="DD.MM.RRRR" className="w-full bg-slate-950 border border-rose-900/50 rounded-xl px-4 py-3 text-rose-400 font-bold outline-none focus:border-rose-500" />
                </div>
            )}

            <div className="grid grid-cols-2 gap-4"><div><label className="block text-xs font-bold text-slate-500 mb-1 uppercase tracking-wider">Typ Smlouvy</label><select value={contractForm.type} onChange={e => setContractForm({...contractForm, type: e.target.value})} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white outline-none focus:border-blue-500"><option value="HPP">Pracovní smlouva (HPP)</option><option value="DPP">Dohoda (DPP)</option><option value="NDA">Dohoda o mlčenlivosti (NDA)</option><option value="PROTOCOL">Předávací protokol</option></select></div><div><label className="block text-xs font-bold text-slate-500 mb-1 uppercase tracking-wider">Odměna (CZK/Měsíc)</label><input type="number" value={contractForm.salary} onChange={e => setContractForm({...contractForm, salary: e.target.value})} placeholder="Např. 50000" className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white outline-none focus:border-blue-500" /></div></div><div><label className="block text-xs font-bold text-slate-500 mb-1 uppercase tracking-wider">Pracovní Pozice / Předmět</label><input type="text" value={contractForm.position} onChange={e => setContractForm({...contractForm, position: e.target.value})} placeholder="Např. Software Inženýr" className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white outline-none focus:border-blue-500" /></div><button type="submit" disabled={isProcessing} className="mt-4 w-full py-4 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-800 rounded-xl font-bold transition-colors flex items-center justify-center gap-2"><Lock size={18} /> {isProcessing ? 'Šifruji...' : 'Zašifrovat a odeslat'}</button></form></div>
            ...
            <div className="space-y-6 animate-fade-in">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 flex flex-col items-center justify-center shadow-xl">
                        ...
                        <h3 className="text-slate-400 text-xs font-bold uppercase tracking-wider text-center">Nedotknutelné</h3>
                    ...
                    <div className="bg-slate-900 p-6 rounded-3xl border border-blue-900 flex flex-col items-center justify-center shadow-[0_0_20px_rgba(59,130,246,0.15)] relative overflow-hidden">
                        ...
                        <h3 className="text-blue-400 text-xs font-bold uppercase tracking-wider text-center">V ochranné lhůtě</h3>
                        <p className="text-4xl font-extrabold text-white mt-2">{radarData.withdrawal_risk}</p>
                    </div>
                    <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 flex flex-col items-center justify-center shadow-xl">
                        <FileText className="text-amber-500 mb-2" size={28} />
                        <h3 className="text-slate-400 text-xs font-bold uppercase tracking-wider text-center">Čeká na podpis</h3>
                        <p className="text-4xl font-extrabold text-white mt-2">{radarData.pending_signatures}</p>
                    </div>
                    
                    {/* NOVÉ BLOKY EXPIRACÍ */}
                    <div className="bg-slate-900 p-6 rounded-3xl border border-orange-900/50 flex flex-col items-center justify-center shadow-xl">
                        <Clock className="text-orange-500 mb-2" size={28} />
                        <h3 className="text-orange-400 text-xs font-bold uppercase tracking-wider text-center">Končí do 30 dnů</h3>
                        <p className="text-4xl font-extrabold text-white mt-2">{radarData.expiring_soon || 0}</p>
                    </div>
                    <div className="bg-slate-900 p-6 rounded-3xl border border-rose-900/50 flex flex-col items-center justify-center shadow-xl">
                        <AlertOctagon className="text-rose-500 mb-2" size={28} />
                        <h3 className="text-rose-400 text-xs font-bold uppercase tracking-wider text-center">Expirované</h3>
                        <p className="text-4xl font-extrabold text-white mt-2">{radarData.expired || 0}</p>
                    </div>
                </div>
                ...
                <div className="bg-slate-900 p-8 rounded-3xl border border-slate-800 shadow-xl">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <button onClick={handleDownloadAudit} className="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-blue-600 text-white rounded-xl text-sm font-bold transition-colors"><FileText size={16}/> Stáhnout certifikovaný audit</button>
                    </div>
                    <div className="max-h-96 overflow-y-auto space-y-3">
                        {radarData.details && radarData.details.length > 0 ? ( radarData.details.map((detail, idx) => (
                            <div key={idx} className="flex justify-between items-center bg-slate-950 border border-slate-800 p-4 rounded-xl">
                                ...
                                <span className={`px-3 py-1 text-xs font-bold rounded-lg uppercase border 
                                    ${ detail.state === 'LOCKED_SAFE' ? 'bg-emerald-900/30 text-emerald-400 border-emerald-500/30' : 
                                       ...
                                       detail.state === 'EXPIRING_SOON' ? 'bg-orange-900/30 text-orange-400 border-orange-500/30' : 
                                       detail.state === 'EXPIRED' ? 'bg-rose-900/30 text-rose-400 border-rose-500/30' : 
...
print("[+] Přepsání dokončeno: Logika pro Doby určité/neurčité s předvyplňováním a rozšířený Právní Radar byly úspěšně přidány!")
